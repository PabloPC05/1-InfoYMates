...
private:
    ...
    std::ofstream tableroUsado; //Archivo de tablero generado
    ...
    void escribirTablero();  //Escribe el tablero en el archivo solucion despues de generarlo
...
public:
    Juego(int fM = 0, int fC = 0, Estado E = JUGANDO) { fichaMeta.first = fM; fichaMeta.second = fC; estadoInt = E; }  //Constructor

    //Getter del estado del juego
    Estado getEstado() { return estadoInt; }

    void generarJuegoAleatorio(int f, int c);  //Funcion directora de la generacion del tablero

    bool cargar(std::istream& entrada); //Funcion directora de la carga el tablero de una entrada x

    //Metodos de juego
    Movimiento leerMovimiento(); //Lee un movimiento del teclado
